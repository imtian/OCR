...
public class ImageUtils {
	...
	public ImageUtils(BufferedImage srcImage){
	 ...
	 * @return {@link ImageUtils}
	 */
	public static ImageUtils fromImage(BufferedImage srcImage){
		ImageUtils image = new ImageUtils(srcImage);
		image.init();
		return image;
	}
    ...
    public BufferedImage resize() {
    	return copy(keepImageRatio(resize(srcImage), givenWidth, givenHeight), BufferedImage.TYPE_INT_RGB);
    }
    ...
    public void makeImage(BufferedImage newImage,  byte[] buffer) throws IOException{
		newImage = this.copy(newImage, BufferedImage.TYPE_INT_RGB);
		...
		ImageOutputStream outputStream = ImageIO
				.createImageOutputStream(buffer);
		imgWriter.setOutput(outputStream);
		IIOImage outputImage = new IIOImage(newImage, null, null);
		imgWriter.write(null, outputImage, imgWriteParam);
		imgWriter.dispose();
